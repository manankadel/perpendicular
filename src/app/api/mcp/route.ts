...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (name === "employee_run") {
      ...
      const state = await updateWorkspace(identity.context.workspaceId, (workspace) => {
        workspace.runs.unshift({ id: createId("run"), employeeId, trigger: "manual", task, output: generated.content, score: 80, reason: "Run executed through the MCP command surface.", status: "completed", createdAt: timestamp(), durationMs: 0, trace: [{ label: "Worker", detail: generated.provider, durationMs: 0, cost: 1, status: "complete" }] });
