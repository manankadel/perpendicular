...
const document = {
  ...
  paths: {
    ...
    "/api/workspace": {
      ...
      post: { description: "Execute a workspace command. All commands require workspace:write because they can persist state or consume credits. Actions: bootstrap-workspace, run-onboarding-brief, enable-onboarding-schedule, chat, create-employee, create-document, create-list, import-row, run-employee, evaluate-employee, activate-prompt-version, schedule-employee, enrich-row, enroll-row, suppress-row, unsuppress-row, create-sequence, create-ticket, resolve-ticket, rate-ticket.", responses: { "200": { description: "Updated workspace state." }, "403": { description: "Workspace write permission required." } } },
    },
    ...
    "/api/inbox": { get: { description: "List workspace-scoped persisted Gmail messages after a sync. Requires workspace:read for API keys.", parameters: [{ name: "limit", in: "query", schema: { type: "integer", minimum: 1, maximum: 100 } }], responses: { "200": { description: "Persisted Gmail messages." }, "403": { description: "Workspace read permission required." }, "503": { description: "Inbox migrations are not available." } } } },
    ...
    "/api/integrations/google/callback": { get: { security: [], description: "OAuth callback that validates the one-time state, encrypts the Gmail refresh token, and returns to the canonical Perpendicular UI.", responses: { "302": { description: "Return to the canonical UI with a connection result." } } } },
    ...
    "/api/integrations/gmail/sync": { post: { description: "Pull Gmail messages using the persisted history cursor, persist them, and pause matching enrolled rows on replies. Requires settings:write for API keys because synchronization mutates workspace state.", responses: { "200": { description: "Inbox synchronized." }, "403": { description: "Settings write permission required." } } } },
    ...
    "/api/keys/{id}": { delete: { description: "Revoke an active API key. Requires settings:write and never returns the plaintext secret.", responses: { "200": { description: "Key revoked." }, "403": { description: "Settings write permission required." }, "404": { description: "Key not found." } } } },
    "/api/mcp": { post: { description: "MCP JSON-RPC endpoint exposing workspace_get and integrations_list with workspace:read, plus employee_chat and employee_run with workspace:write.", responses: { "200": { description: "JSON-RPC response." } } } },
    "/api/pricing": { get: { security: [], responses: { "200": { description: "Truthful self-hosted pricing metadata." } } } },
    "/api/cron/heartbeat": { post: { security: [], description: "Authenticated host-cron worker entrypoint. Requires CRON_SECRET and claims due heartbeat jobs with Postgres leases.", responses: { "200": { description: "Heartbeat work completed or skipped as already claimed." }, "401": { description: "Cron secret is invalid." }, "503": { description: "Cron secret is not configured in production." } } } },
  },
};
